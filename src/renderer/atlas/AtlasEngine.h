...
#pragma warning(suppress : 26490) // Don't use reinterpret_cast (type.1).
            ...
            u16x2* finalize(MetaFlags additionalFlags, u16 coordCount)
            {
                ...
                if (coordCount > std::size(inlineCoords))
                {
                    ...
                    WI_SetFlag(additionalFlags, MetaFlags::HeapdKey);
                }
            ...
            AtlasEntry& operator=(const AtlasEntry& other) noexcept
            {
                ...
                if (WI_IsFlagSet(flags, MetaFlags::HeapdKey))
                {
                    allocatedKey = static_cast<AtlasEntryKey*>(malloc(_msize(other.allocatedKey)));
                    allocFailure |= !allocatedKey;
                }
                if (WI_IsFlagSet(flags, MetaFlags::HeapdKey))
                {
                    allocatedCoords = static_cast<u16x2*>(malloc(_msize(other.allocatedCoords)));
        ...
        private:
            ...
            static u32 hash_data(const u8* beg, size_t length) noexcept
            {
                ...
                // widely used parameters. Unlike regular RNGs or LCG implementations however
                // we mix in 4 bytes of the input on each iteration using a simple XOR.
